...
float dif_D1=0.64f; //Падение напряжение на диоде D1, лучше диод шотки поставить.
...
//---------------------------------------------DIODE------------------------------------------------  
  if(PartFound == PART_DIODE) {
	...
	if(diodes[0].Anode == diodes[1].Anode) {			// Common Anode
	  lcd_eep_string(DualDiode);					// Message - "Double diode Ђ"
	...
	if((b<3) && (c<3)) {
	  lcd_eep_string(TwoDiodes);					// Message - "2 diodes"
	  Line2();							// Start second line
	  lcd_eep_string(InSeries);					// Message - "serial A=ЂЂ"
...
//---------------------------------------------RESISTOR----------------------------------------------	  
	      ...
	      lcd_eep_string(Resistor);					// Message - "Resistor: ЂЂ"
...
//---------------------------------------------CAPACITOR---------------------------------------------	    
		...
		lcd_eep_string(Capacitor);				// Message - "Capacitor: ЂЂ"
  ...
  if(adcv[0] < 200) {							// If the Device does not have a passage between HighPin and LowPin
	...
	ADC_PORT = 0x00;
		...
		/*      Without unloading it can come to wrong identifications, since the gate of a MOSFETs can be still loaded. 
		        The additional measurement with " groЯen" R_H resisted is accomplished, around antiparallel diodes of 
...
//  Тактовая частота ADPS2:0, ADEN-Разрешение АЦП, ADIE-Разрешение прерывания от АЦП, ADATE-0-1кратное преобразование. 1-непрерывное преобразование. 
